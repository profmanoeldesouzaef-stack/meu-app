...
export const LoginModal: React.FC = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (!cleanEmail) {
      setFeedback({
        text: "Por favor, digite seu e-mail cadastrado para redefinir a senha.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      console.log("[AUTH] Solicitando redefinição de senha para:", cleanEmail);
      const { error } = await supabase.auth.resetPasswordForEmail(cleanEmail, {
        redirectTo: "https://vyratraining.com/redefinir-senha",
      });

      if (error) {
        console.warn("[AUTH] Erro resetPasswordForEmail:", error);
        setFeedback({
          text: error.message || "Erro ao solicitar redefinição de senha.",
          type: "error",
        });
      } else {
        setFeedback({
          text: `Enviamos o link de redefinição para ${cleanEmail}! Verifique sua caixa de entrada e pasta de spam.`,
          type: "success",
        });
      }
    } catch (err: any) {
      console.error("[AUTH] Exceção em resetPasswordForEmail:", err);
      setFeedback({
        text: err?.message || "Erro inesperado ao solicitar redefinição de senha.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0A0A0A] flex items-center justify-center p-4 overflow-y-auto">
      <div className="w-full max-w-md my-auto rounded-3xl bg-[#151515] border border-[#2B2B2F] p-6 sm:p-8 space-y-6 text-center shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        {/* Abas Entrar / Cadastrar (exibidas apenas se não estiver em recuperação) */}
        {mode !== "forgot_password" ? (
          ...
          <div className="flex items-center justify-between px-1">
            <button
              ...
              className="text-xs text-[#9B9BA1] hover:text-[#F5F5F7] flex items-center gap-1.5 transition-colors cursor-pointer font-medium"
            >
              <ArrowLeft className="w-4 h-4" />
              <span>Voltar ao login</span>
            </button>
            <span className="text-[11px] font-bold text-[#FF6A2A] uppercase tracking-wider">
          ...
          {mode !== "forgot_password" && (
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-[11px] font-bold text-[#9B9BA1] uppercase tracking-wider">
                  Senha
                </label>
                {mode === "login" && (
                  <button
                    ...
                    className="text-[11px] font-semibold text-[#FF6A2A] hover:text-[#E85B1F] transition-colors cursor-pointer hover:underline"
                  >
                    Esqueci minha senha
                  </button>
                )}
        ...
        {/* Divisor e redes sociais exibidos apenas para login e cadastro */}
        {mode !== "forgot_password" && (
          <>
